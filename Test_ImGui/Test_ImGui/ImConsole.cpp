...
void ImConsole::Draw(const char* title, bool* p_open)
{
    ...
    if (ImGui::InputText("Input", input_buffer, IM_ARRAYSIZE(input_buffer), input_text_flags, TextEditCallback, (void*)this))
    {
        char* s = input_buffer;
        Strtrim(s);
        if (s[0])
...
void ImConsole::ExecuteCommand(const char* command)
{
    AddLog("# %s\n", command);
...
void ImConsole::AddLog(const char* fmt, ...)
{
	char buffer[1024];
	va_list args;
	va_start(args, fmt);
...
int ImConsole::TextEditCallback(ImGuiInputTextCallbackData* data)
{
    ...
    {
        // Example of HISTORY
        const int prev_history_position = history_position;
        ...
        if (prev_history_position != history_position)
        {
            const char* history_string = (history_position >= 0) ? history[history_position] : "";
            data->DeleteChars(0, data->BufTextLen);
            data->InsertChars(0, history_string);
        }
    }
    }
    return 0;
